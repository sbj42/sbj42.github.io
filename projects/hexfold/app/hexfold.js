...
function onMouseUp(event) {
    ...
    if (connections == piececount * 2 && !unfulfilled) {
        ...
        for (var i = curlevel + 1; i < 37 && i < curlevel + 3; i ++)
            if (levelstate[i] == 'l')
                levelstate[i] = 'o';
        chrome.storage.sync.set({'levels': levelstate.join('')});
        if (levels[curlevel].last) {
            $('#next').hide();
            $('#again').show();
        } else {
...
$(document).ready(function() {
    chrome.storage.sync.get('levels', function(data) {
        levelstate = (data.levels || 'ollllllllllllllllllllllllllllllllllll').split('');
        ...
        $('#l_restart').click(reset);
        $('#l_skip').click(skip);
        $('#l_levels, #l_levels2').click(levelselect);
        $('#next').click(next);
        $('#again').click(reset);
